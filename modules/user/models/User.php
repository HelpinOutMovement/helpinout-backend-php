...
// for arise
use app\models\City;
use app\modules\arise\models\AriseOrganization;
use app\modules\arise\models\AriseProject;
use app\modules\arise\models\AriseUserCity;
use app\modules\arise\models\AriseUserOrganization;
use app\modules\arise\models\AriseUserSchool;
...
class User extends ActiveRecord implements IdentityInterface {
    ...
    public function rules() {
        return [
            // username rules
            ['username', 'required', 'on' => ['register', 'connect', 'create', 'update', 'add', 'centre']],
            ['username', 'match', 'pattern' => '/^[a-zA-Z]\w+$/'],
            ['username', 'string', 'min' => 3, 'max' => 25],
            ['username', 'unique'],
            ['username', 'trim'],
            // email rules
            ['email', 'required', 'on' => ['register', 'connect', 'create', 'update', 'add']],
            ['email', 'email'],
            ...
            ['password', 'string', 'min' => 6, 'on' => ['register', 'create', 'add', 'update']],
        ];
    }
    ...
    private $_stateRole = null;

    public function getRole() {
        $roles = \Yii::$app->authManager->getRolesByUser(\yii::$app->user->identity->id);

        if ($this->_stateRole === null && $roles !== null) {
            foreach ($roles as $rol) {
                $this->_stateRole = $rol->name;
            }
        }
        return $this->_stateRole;
    }
    ...
    private $_stateRoledisplay = null;

    public function getRoledisplay() {
        $roles = \Yii::$app->authManager->getRolesByUser(\yii::$app->user->identity->id);

        if ($this->_stateRoledisplay === null && $roles !== null) {
            foreach ($roles as $rol) {
                $this->_stateRoledisplay = $rol->description;
            }
        }
        return $this->_stateRoledisplay;
    }

    public function setRoledisplay() {
        return $this->_stateRoledisplay;
    }

    private $_stateProject = null;

    public function getProject() {
        $roles = \Yii::$app->params['role'];
        if ($this->_stateProject === null && $roles !== null) {
            foreach ($roles as $key => $value) {
                if (is_array($value)) {
                    if (key_exists(yii::$app->user->identity->role, $value)) {
                        $this->_stateProject = $key;
                    }
                }
            }
        }
        return $this->_stateProject;
    }
    ...
    public function getCity() {
        return $this->hasOne(AriseUserCity::className(), ['user_id' => 'id']);
    }

    public function getOrg() {
        return $this->hasOne(AriseUserOrganization::className(), ['user_id' => 'id']);
    }

    public function getSchool() {
        // return $this->hasMany(AriseUserSchool::className(), ['user_id' => 'id'])->viaTable(AriseUserOrganization::tableName(), ['organization_id' => 'organization_combined_id']);
        return $this->hasMany(AriseUserSchool::className(), ['user_id' => 'id']);
    }

    public function getCities() {
        // return $this->hasMany(AriseUserSchool::className(), ['user_id' => 'id'])->viaTable(AriseUserOrganization::tableName(), ['organization_id' => 'organization_combined_id']);
        return $this->hasMany(\app\modules\shikshaantar\models\UserShikshaantarCity::className(), ['user_id' => 'id']);
    }

    public function getCentres() {
        // return $this->hasMany(AriseUserSchool::className(), ['user_id' => 'id'])->viaTable(AriseUserOrganization::tableName(), ['organization_id' => 'organization_combined_id']);
        return $this->hasMany(\app\modules\shikshaantar\models\UserShikshaantarCenter::className(), ['user_id' => 'id']);
    }

    public static function getUserUrl() {
        if (yii::$app->user->identity->role == 'superadmin' || yii::$app->user->identity->role == 'tliteam')
            $urlpre = '';
        else
            $urlpre = '/' . yii::$app->user->identity->project;
        return $urlpre;
    }
    ...
    public static function userIdentifierCallback($id) {
        ...
        return $user ? yii\helpers\Html::a($user->username, ['/user/admin/update', 'id' => $user->id]) : $id;
    }
